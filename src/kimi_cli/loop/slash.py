...
from kimi_cli.eventbus.types import StatusUpdate, TextPart
from kimi_cli.loop import bus_send
from kimi_cli.loop.agent import load_project_agents_md
...
type AgentLoopSlashCmdFunc = Callable[[KimiAgentLoop, str], None | Awaitable[None]]
"""
A function that runs as a KimiAgentLoop-level slash command.

Raises:
    Any exception that can be raised by `AgentLoop.run`.
"""

registry = SlashCommandRegistry[AgentLoopSlashCmdFunc]()
...
async def add_dir(soul: KimiAgentLoop, args: str):
    ...
    if not await path.exists():
        bus_send(TextPart(text=f"Directory does not exist: {path}"))
        return
    if not await path.is_dir():
        bus_send(TextPart(text=f"Not a directory: {path}"))
        return
    ...
    try:
        ...
        bus_send(TextPart(text=f"Cannot read directory: {path} ({e})"))
        return
