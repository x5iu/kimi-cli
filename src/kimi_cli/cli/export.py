...
from kimi_cli.eventbus.log import EventLogMetadata, parse_event_log_line
from kimi_cli.eventbus.types import TurnBegin

if TYPE_CHECKING:
    from kimi_cli.session import Session

_LOG_RETENTION_SECONDS = 2 * 24 * 60 * 60
_MAX_LOG_BYTES = 100 * 1024 * 1024


def attach_export_command(app: typer.Typer) -> None:
    app.command(name="export", help="Export a session as a ZIP archive.")(export_command)
...
def _last_user_message_timestamp(session_dir: Path) -> float | None:
    wire_file = _event_log_path(session_dir)
...
def _build_manifest(session_id: str, session_dir: Path) -> dict[str, str | None]:
    ...
    try:
        ...
        manifest["kimi_cli_version"] = VERSION
