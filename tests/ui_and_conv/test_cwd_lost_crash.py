...
from kimi_cli.loop import AgentLoop, StatusSnapshot
...
async def test_print_cwd_lost_crash_shows_unknown_for_non_kimi_loop(
    ...
    cap = Console(
        file=StringIO(), width=120, record=True, force_terminal=True, color_system="truecolor"
    )
...
async def test_print_cwd_lost_crash_shows_session_for_kimi_loop(
    ...
    shell = Shell(soul)
    cap = Console(
        file=StringIO(), width=200, record=True, force_terminal=True, color_system="truecolor"
    )
    monkeypatch.setattr(shell_module.console, "print", cap.print)
    shell._print_cwd_lost_crash()
    out = cap.export_text()
...
async def test_shell_run_breaks_on_getcwd_oserror(
    ...
    monkeypatch.setattr(
        shell_module.os, "getcwd", lambda: (_ for _ in ()).throw(OSError(2, "ENOENT"))
    )
    cap = Console(
        file=StringIO(), width=120, record=True, force_terminal=True, color_system="truecolor"
    )
...
def test_working_dir_text_getcwd_oserror_requests_app_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    class _StubApp:
        ...
        def exit(
            self, *, exception: BaseException | None = None, result: object | None = None
        ) -> None:
    ...
    stub = _StubApp()
    s = CPS(
        status_provider=lambda: StatusSnapshot(
...
def test_working_dir_text_getcwd_oserror_without_app_returns_empty(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from kimi_cli.ui.shell.prompt import CustomPromptSession as CPS
