...
async def list_directory(work_dir: KaosPath) -> str:
    ...
    Both depth and width are capped to keep the system-prompt token budget bounded:
    * Root: up to _LIST_DIR_ROOT_WIDTH entries.
    * Children of root dirs: up to _LIST_DIR_CHILD_WIDTH entries per directory.
    * Truncated levels show ``... and N more``.
    """
    lines: list[str] = []
