...
from kimi_cli.eventbus.log import BusMessageRecord
from kimi_cli.eventbus.serde import deserialize_bus_message, serialize_bus_message
...
def _test_serde(msg: BusMessage):
    serialized = serialize_bus_message(msg)
    deserialized = deserialize_bus_message(serialized)
    assert deserialized == msg


async def test_wire_message_serde():
    """Test serialization of all BusMessage types."""

    msg = TurnBegin(user_input="Hello, world!")
    assert serialize_bus_message(msg) == snapshot(
    ...
    msg = TurnEnd()
    assert serialize_bus_message(msg) == snapshot({"type": "TurnEnd", "payload": {}})
    _test_serde(msg)

    msg = StepBegin(n=1)
    assert serialize_bus_message(msg) == snapshot({"type": "StepBegin", "payload": {"n": 1}})
    _test_serde(msg)

    msg = StepInterrupted()
    assert serialize_bus_message(msg) == snapshot({"type": "StepInterrupted", "payload": {}})
    _test_serde(msg)

    msg = CompactionBegin()
    assert serialize_bus_message(msg) == snapshot({"type": "CompactionBegin", "payload": {}})
    _test_serde(msg)

    msg = CompactionEnd()
    assert serialize_bus_message(msg) == snapshot({"type": "CompactionEnd", "payload": {}})
    _test_serde(msg)

    msg = MCPLoadingBegin()
    assert serialize_bus_message(msg) == snapshot({"type": "MCPLoadingBegin", "payload": {}})
    _test_serde(msg)

    msg = MCPLoadingEnd()
    assert serialize_bus_message(msg) == snapshot({"type": "MCPLoadingEnd", "payload": {}})
    _test_serde(msg)
...
def test_bad_wire_message_serde():
    with pytest.raises(ValueError):
        deserialize_bus_message(None)

    with pytest.raises(ValueError):
        deserialize_bus_message([])
...
def test_wire_message_type_alias():
    import kimi_cli.eventbus.types

    module = kimi_cli.eventbus.types
    # Helper types that are BaseModel subclasses but not BusMessage types
    _NON_WIRE_TYPES = {BusMessageEnvelope, QuestionOption, QuestionItem, QuestionResponse}
...
def test_read_bus_lines_request_id(tmp_path: Path):
    ...
    # Parse
    lines = read_bus_lines(event_log)
    assert len(lines) == 3
