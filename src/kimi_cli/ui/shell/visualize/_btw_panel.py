...
class _BtwModalDelegate:
    ...
    def render_running_prompt_body(self, columns: int) -> ANSI:
        ...
        if self._is_loading:
            ...
            parts.append(Text(""))
            lines = self._response.splitlines()
            if len(lines) <= _BTW_SHORT_ANSWER_LINES:
                parts.append(Text("Enter or Escape to dismiss", style="dim"))
            else:
                parts.append(Text("↑/↓ scroll · Enter or Escape dismiss", style="dim"))
        else:
        ...
        hint_parts.append("↑/↓ scroll · Enter or Escape dismiss")
