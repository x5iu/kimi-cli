...
def test_steer_rejects_unsupported_content_parts(tmp_path) -> None:
    """Steer should validate content-part capabilities like Shell does."""
    scripts = [
        "\n".join(
            [
                "text: working",
    ...
    try:
        ...
        steer_resp = normalize_response(read_response(wire, "steer-unsupported"))
        assert steer_resp["error"]["code"] == -32002
        assert "image_in" in steer_resp["error"]["message"]
